...
class ADMM: #ADMM class (but also used for SLR training)
    def __init__(self, args, task, model, policy_params, file_name, rho=0.1):
    ...
    def init(self, args, config, model):
        ...
        """
        # for k in self.policy_params.keys():
        #     print(k)
        ...
        with open(config, "r") as stream:
            try:
                ...
                ### TODO: add skip layer in prune dict
                for (name, W) in model.named_parameters():
                    if (name not in self.prune_ratios) or \
                        (self.task not in name):
                        continue
                    # policy_name = name.split("taskOp")[0]+"policy."+name.split(".")[-2]
                    if "conv" in name:
                        tmp = name.split(".")[6]
                    else:
                        tmp = name.split(".")[5]
                    policy_name = name.split(tmp)[0]+"policy."+self.task
                    # specific = torch.equal(self.policy_params[policy_name], torch.tensor([0.0, 1.0, 0.0]).cuda())
                    # if not specific:   # only prune "specific" layer
                    #     continue
                    shared = torch.equal(self.policy_params[policy_name], torch.tensor([1.0, 0.0, 0.0]).cuda())
                    if shared:   # not prune "shared" layer, prune "specific" and "skip"
                        continue
...
def hard_prune(args, tasks, model, config, policy_params, option=None):
    """
    ...
    print("hard pruning")
    for (name, W) in model.named_parameters():
        ### TODO: add skip to prune dict
        if (name not in prune_ratios) or \
            ((tasks[0] not in name) and (tasks[1] not in name)):
            continue
        ...
        if "conv" in name:
            tmp = name.split(".")[6]
            policy_name = name.split(tmp)[0]+"policy."+name.split(".")[7]
        else:
            tmp = name.split(".")[5]
            policy_name = name.split(tmp)[0]+"policy."+name.split(".")[6]
        # specific = torch.equal(policy_params[policy_name], torch.tensor([0.0, 1.0, 0.0]).cuda())
        # if not specific:   # only prune "specific" layer
        #     continue
        shared = torch.equal(policy_params[policy_name], torch.tensor([1.0, 0.0, 0.0]).cuda())
        if shared:   # not prune "shared" layer, prune "specific" and "skip"
            continue
...
def test_overall_sparsity(model, policy_params):
    ...
    for name, policy in policy_params.items():
        ...
        if shared:
            feature_name = name.split("policy")[0]+"weight"
...
def test_specific_sparsity(tasks, model, config):
    """
    test sparsity for every involved layer and the overall compression rate
    ...
    for task in tasks:
        ...
        for i, (name, W) in enumerate(model.named_parameters()):
            if ('bias' in name) or (name not in prune_ratios) or (task not in name):
                continue
            W = W.cpu().detach().numpy()
            zeros = np.sum(W == 0)
            total_zeros += zeros
...
def test_sparsity(args, model, config, policy_params):
    """
    test sparsity for every involved layer and the overall compression rate
    ...
    if args.sparsity_type == "irregular":
        for i, (name, W) in enumerate(model.named_parameters()):
            if ('bias' in name) or ("policy" in name):
                continue
            W = W.cpu().detach().numpy()
            zeros = np.sum(W == 0)
            total_zeros += zeros
        ...
        for i, (name, W) in enumerate(model.named_parameters()):
            ...
            if 'bias' in name or name not in prune_ratios:
                continue
            W = W.cpu().detach().numpy()
            shape = W.shape
...
def z_u_update(args, task, ADMM, model, epoch, batch_idx):
    ...
    if args.optimization == 'admm':
        ...
        if epoch != 1 and (epoch - 1) % args.admm_epoch == 0 and batch_idx == 0:
            for i, (name, W) in enumerate(model.named_parameters()):
                ### TODO: add skip to prune dict
                if (name not in ADMM.prune_ratios) or \
                    (task not in name):
                    continue
                ...
                if "conv" in name:
                    ...
                    tmp = name.split(".")[5]
                policy_name = name.split(tmp)[0]+"policy."+task
                # specific = torch.equal(ADMM.policy_params[policy_name], torch.tensor([0.0, 1.0, 0.0]).cuda())
                # if not specific:   # only prune "specific" layer
                #     continue
                shared = torch.equal(ADMM.policy_params[policy_name], torch.tensor([1.0, 0.0, 0.0]).cuda())
                if shared:   # not prune "shared" layer, prune "specific" and "skip"
                    continue
        ...
        if (epoch != 1) and (batch_idx == 0):
            ...
            satisfied1 = Lagrangian1(task, ADMM, model) #check if surrogate optimality condition is satisfied.
            ...
            satisfied2 = Lagrangian2(task, ADMM, model) #check if surrogate optimality condition is satisfied.
            if has_wandb and args.enable_wandb:
...
def Lagrangian2(task, ADMM, model):
    '''
...
def Lagrangian1(task, ADMM, model):
    '''
    This functions checks Surrogate optimality condition after each epoch. 
    If the condition is satisfied, it returns 1.
    If not, it returns 0.
...
def append_admm_loss(args, task, ADMM, model, ce_loss):
    '''
...
def admm_masked_train(args, task, ADMM, model, device, train_loader, optimizer, epoch):
    model.train()
    masks = {}
    writer = None
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        z_u_update(args, task, ADMM, model, epoch, batch_idx)    # update Z and U variables
...
def combined_masked_retrain(args, task, ADMM, model, device, train_loader, optimizer, epoch):
...
def masked_retrain(args, task, ADMM, model, device, train_loader, optimizer, epoch):
    if not masked_retrain:
        return

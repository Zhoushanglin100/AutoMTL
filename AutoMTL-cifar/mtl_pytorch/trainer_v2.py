...
#             self.train_iter_dict[task] = iter(self.cycle(self.train_dataloader_dict[task][0]))
    ...
    def post_train_prune(self, args, 
        ...
        if args.admm_train:
            ...
            for i in range(args.rho_num):
                ...
                admm_dict = {}
                for task in self.tasks:
                    # ADMM = admm.ADMM(args, self.model, policy_params, 
                    #                  f"profile/{args.config_file}_{task}.yml", 
                    #                  rho=current_rho)
                ...
                for epoch in range(1, args.epochs+1):
                    ...
                    mixed_loss, loss = self.slr_train(args, admm_dict, optimizer, 
                                                      epoch, policy_params, writer)
                ...
                admm.hard_prune(args, self.tasks, model_forhard, 
                                f"profile/{args.config_file}.yml", policy_params)
                admm.test_sparsity(args, self.tasks, model_forhard, policy_params)
                admm.test_overall_sparsity(model_forhard, policy_params)
                self.validate('mtl', epoch, hard=True, writer=writer)
        ...
        if args.masked_retrain:
            ...
            print("\n!!!!!!!!!!!!!!!!!!! RETRAIN PHASE !!!!!!!!!!!!!!!!!!!")
            retrain_dir = savePath+"/slr_retrain/"+args.config_file+"/"
            ...
            admm.hard_prune(args, self.tasks, self.model, 
                            f"profile/{args.config_file}.yml", policy_params)
            # admm.test_sparsity(args, self.tasks, self.model, policy_params)
            # admm.test_overall_sparsity(self.model, policy_params)            
            ...
            admm.test_sparsity(args, self.tasks, self.model, policy_params)
            admm.test_overall_sparsity(self.model, policy_params)
            admm.test_specific_sparsity(self.tasks, self.model, f"profile/{args.config_file}.yml")
    ...
    def slr_train(self, args, admm_dict, optimizer, epoch, policy_params, writer):
        ...
        for task in self.tasks: 
            ...
            for batch_idx, data in enumerate(train_loader):
                ...
                ce_loss, admm_loss, mixed_loss = admm.append_admm_loss(args, task, ADMM, self.model, ce_loss)  # append admm losss

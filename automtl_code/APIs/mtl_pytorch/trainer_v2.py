...
#             self.train_iter_dict[task] = iter(self.cycle(self.train_dataloader_dict[task][0]))
    ...
    def post_train_prune(self, args, loss_lambda=None,
        ...
        cfg_name = args.config_file.split("/")[-1].split(".yml")[0]
        ...
        if args.admm_train:
            ...
            admm_dir = savePath+"/slr_train/"+cfg_name+"/"
            if not os.path.exists(admm_dir):
                os.makedirs(admm_dir)
            hp_dir = savePath+"/slr_hp/"+cfg_name+"/"
            ...
            for i in range(args.rho_num):
                ...
                admm_dict = {}
                for task in self.tasks:
                    ADMM = admm.ADMM(args, task, self.model, policy_params, 
                                     args.config_file, 
                                     rho=current_rho)
                ...
                for epoch in range(1, args.epochs+1):
                    ...
                    mixed_loss, loss = self.slr_train(args, admm_dict, optimizer, epoch, loss_lambda)
                    ...
                    ### save admm-trained model
                    state = {'iter': epoch,
                             'state_dict': self.model.state_dict(),
                             'optimizer': optimizer.state_dict()}
                    # self.save_model(state, f'{cfg_name}_{args.sparsity_type}{args.ext}', admm_dir)
                    torch.save(state, f'{admm_dir}/{cfg_name}_{args.sparsity_type}{args.ext}.model')

                print("----------------> Accuracy after hard-pruning ...")
                model_forhard = self.model
                admm.hard_prune(args, self.tasks, model_forhard, args.config_file)
                # sparsity.test_sparsity(args, self.tasks, model_forhard, policy_params)
                # sparsity.test_overall_sparsity(model_forhard, policy_params)
                self.validate('mtl', epoch, hard=True, writer=writer)

                ### save hard-prune model
                state = {'iter': epoch,
                         'state_dict': self.model.state_dict(),
                         'optimizer': optimizer.state_dict()}
                self.save_model(state, f'{cfg_name}_{args.sparsity_type}{args.ext}', hp_dir)
        ...
        if args.masked_retrain:
            ...
            admm.hard_prune(args, self.tasks, self.model, args.config_file)
            # sparsity.test_sparsity(args, self.tasks, self.model, policy_params)
            # sparsity.test_overall_sparsity(self.model, policy_params)            
            print("\n---------------> Accuracy after hard-pruning")
            test_loss_hp = self.validate('mtl', 0, hard=True, writer=writer)
            # ------------------
            ...
            for epoch in range(1, args.retrain_epochs+1):
                ...
                avg_loss = self.masked_retrain(args, admm_dict, optimizer, epoch)
                for param_group in optimizer.param_groups:
                    print("Current lr: ", param_group['lr'])
                self.reset_train_loss()
                test_loss = self.validate('mtl', epoch, hard=True, writer=writer)
                ...
                ### save model
                if test_loss < test_loss_hp:
                    test_loss_hp = test_loss
                    state = {'iter': epoch,
                            'state_dict': self.model.state_dict(),
                            'optimizer': optimizer.state_dict()}
                    # self.save_model(state, f'{cfg_name}_{args.sparsity_type}{args.ext}', retrain_dir)
                    print("... NEW retrain model saved ...\n")
                    torch.save(state, f'{retrain_dir}/{cfg_name}_{args.sparsity_type}{args.ext}.model')

            print("---------------> After retraining")
            self.validate('mtl', epoch, hard=True, writer=writer)
            sparsity.test_sparsity(args, self.model)
            # sparsity.test_overall_sparsity(mtlmodel, policy_params)
            # sparsity.test_specific_sparsity(tasks, mtlmodel)
            sparsity.test_specific_div_sparsity(self.tasks, self.model)
            sparsity.test_specific_policy_sparsity(self.tasks, self.model, policy_params)
    ...
    def slr_train(self, args, admm_dict, optimizer, epoch, loss_lambda):
        ...
        for task in self.tasks: 
            ...
            train_loader = self.train_dataloader_dict[task][0]
            ...
            for batch_idx, data in enumerate(train_loader):
                ...
                if isinstance(loss_lambda, dict):
                    ce_loss = loss_lambda[task] * ce_loss 
                elif loss_lambda is None:
                    pass
                else:
                    sys.exit('Loss weights (lambda) should be in the type of dictionary.')
                ...
                admm.z_u_update(args, task, ADMM, self.model, epoch, batch_idx)  # update Z and U variables
                ce_loss, admm_loss, mixed_loss = admm.append_admm_loss(args, task, ADMM, 
                                                                       self.model, ce_loss)  # append admm loss
    ...
    def masked_retrain(self, args, admm_dict, optimizer, epoch):
        ...
        for task in self.tasks: 
            ...
            print('[Retrain Epoch {}/{} Task {}] Cross_entropy Loss: {:.4f}'.format(epoch, args.retrain_epochs, 
                                                                                    task, avg_loss), flush=True)        
            if self.loss_list[task]:
                avg_loss = np.mean(self.loss_list[task])
            else:
                continue
